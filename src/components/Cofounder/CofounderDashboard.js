...
export default function CofounderDashboard(props) {
	...
	const submitForm = (e) => {
		...
		request
			...
			.send({
				whoAMI: formData.whoAMI.value,
				workExperience: formData.workExperience.value,
				myEducationBackground: formData.myEducationBackground.value,
				languageSpeak: formData.languageSpeak.value,
				impactfulCoFounder: formData.impactfulCoFounder.value,
				rightTimecoFounder: formData.rightTimecoFounder.value,
				UNSustainableDev: formData.UNSustainableDev.value,
				somethingExceptional: formData.somethingExceptional.value
			})
			.then((res) => {
				if (res.status === 200) {
	...
	return (
		<Container>
			...
			<LeftSide>
				<div>
					<h3>3.{user.firstName}'s Profile form</h3>
				</div>
			</LeftSide>
			<br />
			...
			<RightSide>
				<form onSubmit={submitForm}>
					<label>
						Who am I :
						{formData.whoAMI.shouldShowError &&
						!formData.whoAMI.validated && (
							<span
								css={css`
									font-weight: 800;
									color: red;
